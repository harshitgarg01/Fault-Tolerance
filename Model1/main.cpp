...
void particularMethodExecutor(MethodParameters* instance, int index)
{
	...
	while (!inputEnd || !(totalInputTaken == numberInputCompleted))
	{
		...
		if (numberInputCompleted + 1 <= (int)identifierStorage.size())
		{
			...
			{
				Method1 worker;
				arrayOfThreads[threadNumber] = thread(randomSeedSetUp<Method1>, instance, numberInputCompleted, availableThread, threadClosure, threadNumber, index, worker);
			}
			else if (index == 1)
			{
				Method2 worker;
				arrayOfThreads[threadNumber] = thread(randomSeedSetUp<Method2>, instance, numberInputCompleted, availableThread, threadClosure, threadNumber, index, worker);
			}
			else if (index == 2)
			{
				Method3 worker;
				arrayOfThreads[threadNumber] = thread(randomSeedSetUp<Method3>, instance, numberInputCompleted, availableThread, threadClosure, threadNumber, index, worker);
			}
			else if (index == 3)
			{
				Method4 worker;
				arrayOfThreads[threadNumber] = thread(randomSeedSetUp<Method4>, instance, numberInputCompleted, availableThread, threadClosure, threadNumber, index, worker);
			}
			else if (index == 4)
			{
				Method5 worker;
				arrayOfThreads[threadNumber] = thread(randomSeedSetUp<Method5>, instance, numberInputCompleted, availableThread, threadClosure, threadNumber, index, worker);
			}
			else if (index == 5)
			{
				Method6 worker;
				arrayOfThreads[threadNumber] = thread(randomSeedSetUp<Method6>, instance, numberInputCompleted, availableThread, threadClosure, threadNumber, index, worker);
			}
