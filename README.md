...
Through the graphs for attempts vs identifier for scenario 1 for all the models we can clearly see the effect and benefit of exponential backoff retry logic. When we don't use exponential backoff logic in model1 we see that during the degradation period of 60 sec that begins at 84 we see that number of attempts the identifier can make can go as high as 815 which is substantially higher than what what we see with exponential backoff on the same model where it only goes as high as 31 which even the methods without degradation state might go due to occurance of an odd identifier and randomness of the methods. We can clearly see that in model1 with expo backoff the no of retries attempt got restricted to just 24 for identifier 84 as compared to 815. When we compare the invocation time it is quite similar with it being 54 for non-exponential case and 61 for exponential case for identifier 84 that can be seen in excel files. Thus it is easy to conclude from here that applying exponential retry logic helps in drastically reducing the number of attempts with nearly similar invocation time henceforth saving a lot of resources. Both Model2 and Model3 too also agree to this pattern with their attempts being also lower due to exponential backoff logic.

**Next we compare the invocation time among many models.**

**Model1 vs Model2**

In case of independent methods model1 appears to be way better than model2 when judging on invocation time. Let us consider our scenario 2 to compare both. We see that in case of model1 upon degradation state first only the invocation time of method2 rises while for model2 all the methods see a rise in invocation time. The parallel arrangement in model1 helps other methods to continue execution without being hindered by the degraded state of method2. Thus in case of independency of methods it is way better to prefer model1 in compared to model2.

**Model2 vs Model3**

Model3 is just an extended version of model1 with using a global vector to introduce dependency among methods. Now model3 is better than model2 when looked on basis of invocation time as we see in scenario2 in case of model3 we see that method1 which is not dependent on either method2 has a very low invocation time similar to model1. This is again achieved due to parallel working that enables method1 to continue working inspite of dergradation state of method2 in model3 but not in model2. Also we can see if we have two different methods with long degradation state one after another than it can cause to a very high invocation time for the independent methods in model2 while model3 would not face any such trouble. 

The only advantage that model2 has is the low number of extra threads as compared to model3. For Example suppose n is the number of methods in a program(in the problem statement n is 6) and let r be the number of identifiers we want to run parallel at a time(in the given models r is assumed to be at max 10). Then number of extra threads is model2 are of the form r + n while number of threads in model3 are of the form (r+1) x n. Thus if for a high scale project big values or r and n can lead to a very large thread pool which might be difficult to handle in comparison to model2 where the thread pool would be comparitively quite small.
...
Hence we conclude that retry exponential backoff logic plays a vital role in the fault tolerance problem and must be incorporated in the program at all cost. Also that under independency of methods Model1 appears to be the best approach towards approaching the problem while in case of dependency of methods model3 appears to be the most appropriate and best choice.
